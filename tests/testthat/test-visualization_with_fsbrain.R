...
test_that("We can show a coordinate and the vertex closest to it.", {
    ...
    if(requireNamespace("fsbrain", quietly = TRUE)) {
        ...
        ccv = coord_closest_vertex(query_coords, get_surface(bp)); # get closest vertex.
        ccv_coords = get_surface_coords(bp, ccv$both_closest_vertex, ccv$both_hemi); # find coords of closest vertex.
